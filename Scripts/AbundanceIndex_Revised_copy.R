...
data.table::setnames(median.monthly.flow.raw, old = c("Year","March", "April", "May", "June", "July", "August",
                     ...
                     new = c('year','3','4','5','6','7','8','9','10','11'))
## Converting from wide to long format and arranging by year and month
 median.monthly.flow <- gather(median.monthly.flow.raw, month, median.monthly.flow, "3":"11", factor_key = TRUE)
...
max.monthly.flow <- gather(max.monthly.flow.raw, month, max.monthly.flow, "3":"11", factor_key = TRUE)
...
data.table::setnames(monthly.fall.rate.raw, old = c("Year","March", "April", "May", "June", "July", "August",
                     ...
                     new = c('year','3','4','5','6','7','8','9','10','11'))
## Converting from wide to long format
monthly.fall.rate <- gather(monthly.fall.rate.raw, month, monthly.fall.rate, "3":"11", factor_key = TRUE)
...
data.table::setnames(monthly.rise.rate.raw, old = c("Year","March", "April", "May", "June", "July", "August",
                     ...
                     new = c('year','3','4','5','6','7','8','9','10','11'))
## Converting from wide to long format
monthly.rise.rate <- gather(monthly.rise.rate.raw, month, monthly.rise.rate, "3":"11", factor_key = TRUE)
...
data.table::setnames(peak.flow.timing.raw, old = c("Year","March", "April", "May", "June", "July", "August",
                     ...
                     new = c('year','3','4','5','6','7','8','9','10','11'))
## Converting from wide to long format
peak.flow.timing <- gather(peak.flow.timing.raw, month, date.of.monthly.max.flow, "3":"11", factor_key = TRUE)
...
data.table::setnames(monthly.reversals.raw, old = c("Year","March", "April", "May", "June", "July", "August",
                     ...
                     new = c('year','3','4','5','6','7','8','9','10','11'))
## Converting from wide to long format
monthly.reversals <- gather(monthly.reversals.raw, month, monthly.reversals, "3":"11", factor_key = TRUE)
...
environmental.matrix.3monthlag.allvars$date <- as.Date(ifelse(environmental.matrix.3monthlag.allvars$dummy.month <= 12, 
                                                             paste(environmental.matrix.3monthlag.allvars$year,
                                                             environmental.matrix.3monthlag.allvars$dummy.month,
                                                             1, sep = "-"), 
                                                             paste(as.numeric(environmental.matrix.3monthlag.allvars$year) + 1,
                                                                   environmental.matrix.3monthlag.allvars$dummy.month - 12,
                                                                                  1, sep = "-")),
                                                       format = "%Y-%m-%d")
...
environmental.matrix.2monthlag.allvars$date <- as.Date(ifelse(environmental.matrix.2monthlag.allvars$dummy.month <= 12, 
                                                paste(environmental.matrix.2monthlag.allvars$year,
                                                      environmental.matrix.2monthlag.allvars$dummy.month,
                                                      1, sep = "-"), 
                                                paste(as.numeric(environmental.matrix.2monthlag.allvars$year) + 1,
                                                      environmental.matrix.2monthlag.allvars$dummy.month - 12,
                                                      1, sep = "-")),
                                                      format = "%Y-%m-%d")
...
rda_tree.combinedlag <- rda(monthly.abundance_6_9_noHickory ~ env.matrix$fall.rate.3monthlag +
                              ...
                              env.matrix$inflows.lowerqtl.3monthlag + env.matrix$monthly.rise.rate)
rda_tree.combinedlag
RsquareAdj(rda_tree.combinedlag)
plot(rda_tree.combinedlag, type='n', scaling=5)
...
## Testing unlagged predictors for significance

rda_tree.nolag <- rda(monthly.abundance_6_9_noHickory ~ env.matrix$days.over.30k)
rda_tree.nolag
RsquareAdj(rda_tree.nolag)

## Predictor (adj R2)
# Median monthly flow (-0.003232648)
# Max flow (-0.003925715)
# Date of max monthly flow (-0.003098067)
# Fall rate (0.007989048)
#* Rise rate (0.04348975)
# Reversals (0.002770603)
# Inflows (-0.003663886)
# Inflows Q4 (-0.00506694)
# Inflows Q1 (-0.005197903)
# Days over 10k (-0.005336342)
# Days over 15k (-0.001493773)
# Days over 20k (-0.0006986752)
# Days over 30k (-0.004843257)
